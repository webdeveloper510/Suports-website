...
//			
	...
	public function email_sent()
	{
		...
		$cnt_percentage=$count_all_percentage/3;
		$id = $this->session->userdata('user_idd');
		...
		for($i=0;$i<count($my_arr);$i++)
		{
		  ...
		  $content='<div class="outer-email" style="width:60%; border:1px solid #000; margin: 24px; padding: 28px;background: #fff; color: #000; font-family:Open Sans, sans-serif;font-size: 18px;"><img src="https://isportsrecruiting.com/images/black-logo.png" alt="isr_logo" height="63" width="88">'.$_POST['part_first'].'</h3><h3>'.$_POST['second_part'].'</h3><h3 class ="email-con" style="font-size: 18px;   line-height: 24px; font-weight: 600;">'.$_POST['third_part']." "."<b>".$_POST['fourth_part']."</b>"." ".$_POST['fifth_part'].'</h3><h3 style="font-size:18px;font -weight:600;">'.$_POST['sixth_part'].'</h3></div>';
			...
			$this->email->subject($first_name.' '.$last_name.','.$position_name.' '.$state);
   ...
   $data = array(
	...
	{
		$data['find_gpa']= $this->Profile_model->send_blastmail_byselect();
	}
	   ...
	   else
	{
		...
		$gender=$get_pecentage_basic['gender'];
		$birth=$get_pecentage_basic['birth'];
		$weight=$get_pecentage_basic['weight'];
		$height=$get_pecentage_basic['height'];
		$address=$get_pecentage_basic['address'];
		$address2=$get_pecentage_basic['address2'];
		$home_phone=$get_pecentage_basic['home_phone'];
		$cel_phone=$get_pecentage_basic['cel_phone'];
		$email=$get_pecentage_basic['email'];
		$guardian1=$get_pecentage_basic['guardian1'];
		$g_first_name=$get_pecentage_basic['g_first_name'];
		$g_last_name=$get_pecentage_basic['g_last_name'];
		$g_phone=$get_pecentage_basic['g_phone'];
		$g_mail=$get_pecentage_basic['g_mail'];
		...
		$games_per=$get_pecentage_basic['games'];
		$sport_per=$get_pecentage_basic['position_id'];
		$val1=0;
		if($firstnameper1)
		{
			$val1=$val1+1;
		...
		$data['get_ath_history'] = $this->Academic_info_model->athlete_history_percentage();
		if($data['get_ath_history'])
		{
			$val3=$val3+1;
		}
	...
	public function create_profile_submit()
	{
        ...
        $data['sport_name']=$this->Academic_info_model->find_sport_name_profile($sport);
        $sp_name=$data['sport_name']['sport_name'];
...
<tr style='border:1px solid #ccc;'>
   ...
   <td style='border:1px solid #ccc;'>$weight</td>
</tr>
<tr style='border:1px solid #ccc;'>
...
$this->email->set_mailtype("html");
	...
	public function academic_info($add=null)
	{
		...
		else
		{
             ...
             $data['per'] = $this->Academic_info_model->update_acdemic_percentage($percentage);  
	...
	public function athletic_info()
	{
		...
		//print_r($satat);die;
		 $val3=0;
		// print_r($data['video_get']);die;
		 $videeo1=$data['video_get']['video1'];
		 $videeo2=$data['video_get']['video2'];
		if($videeo1)
		{
			$val3=1;
		}
       ...
       if($id4){
		...
		//	$this->load->view('free_header2');
			...
			$data['coach_information'] = $this->Academic_info_model->free_profile_coach_information4($id4);
				...
				$data['generate_form_stats_value'] = $this->Academic_info_model->free_profile_generate_form_stats_value4();
					$data['athlete_info1'] = $this->Academic_info_model->athlete_info1();
